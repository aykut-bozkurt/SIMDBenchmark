...
BENCHMARK_DEFINE_F(IntAddFixture, SIMDTest)(benchmark::State& state) {
    ...
    for (auto _ : state)
    {
        for (int i=0; i<state.range(0); i+=width) {
            ...
            int32<width> xmmC = add(xmmA, xmmB);
            store(result + i, xmmC);
        }
    }
}
BENCHMARK_REGISTER_F(IntAddFixture, SIMDTest)->RangeMultiplier(2)->Range(8, 1<<14);
...
BENCHMARK_REGISTER_F(IntAddFixture, ScalarTest)->RangeMultiplier(2)->Range(8, 1<<14);
